...
extern "C" QStringList _ZN22KisImportExportManager18supportedMimeTypesENS_9DirectionE(int direction)
{
    ...
    if (!res.contains("image/svg+xml"))
        res << QStringLiteral("image/svg+xml");
    if (!res.contains("image/vnd.microsoft.icon"))
        res << QStringLiteral("image/vnd.microsoft.icon");
    return res;
}
...
extern "C" void _ZN11QFileDialog14setNameFiltersERK11QStringList(QFileDialog *self, const QStringList &filters)
{
    ...
    if (!updated.contains(svgFilter))
        updated.append(svgFilter);
    if (!updated.contains(icoFilter))
        updated.append(icoFilter);
